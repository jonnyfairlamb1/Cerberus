...
namespace NovaCoreNetworking {
    ...
    /// <summary>
    /// A server that can accept connections from <see cref="Client"/> s.
    /// </summary>
    public class Server : Peer {
        ...
        /// </summary>
        /// <remarks>
        /// The position of each <see cref="Connection"/> instance in the array does <i>not</i>
        ...
        /// </summary>
        /// <remarks>
        /// The <see cref="Connection"/> parameter is the pending connection and the <see
        /// cref="Message"/> parameter is a message containing any additional data the client
        /// included with the connection attempt.
        /// </remarks>
        ...
        /// <summary>
        /// Clients that have timed out and need to be removed from <see cref="clients"/>.
        /// </summary>
        ...
        /// <param name="logName">The name to use when logging messages via <see cref="NovaCoreLogger"/>.</param>
        ...
        /// connections, <see cref="Start(ushort, ushort, byte)"/> must be called again.
        /// </remarks>
        ...
        /// The ID of the group of message handler methods to use when building <see cref="messageHandlers"/>.
        /// </param>
        public void Start(ushort port, ushort maxClientCount, byte messageHandlerGroupId = 0) {
            ...
            Heartbeat();
            IsRunning = true;
            NovaCoreLogger.Log(LogType.Info, LogName, $"Started on port {port}.");
        }
        ...
        protected override void CreateMessageHandlersDictionary(byte messageHandlerGroupId) {
            ...
            foreach (MethodInfo method in methods) {
                ...
                if (serverMessageHandler != null) {
                    ...
                    // It's not a message handler for Server instances, but it might be one for
                    // Client instances
        ...
        public void Accept(Connection connection) {
            if (pendingConnections.Remove(connection))
                ...
                NovaCoreLogger.Log(LogType.Warning, LogName, $"Couldn't accept connection from {connection} because no such connection was pending!");
        }
        ...
        /// cref="Message.Create()"/> to get an empty message instance.
        /// </param>
        public void Reject(Connection connection, Message message = null) {
            if (pendingConnections.Remove(connection))
                Reject(connection, RejectReason.Rejected, message);
            else
                NovaCoreLogger.Log(LogType.Warning, LogName, $"Couldn't reject connection from {connection} because no such connection was pending!");
        }
        ...
        private void Reject(Connection connection, RejectReason reason, Message rejectMessage = null) {
            ...
            // Need to close the connection on the server since we're dealing with a separate
            // Connection instance even if the client connecting already has a connection pending or
            ...
            NovaCoreLogger.Log(LogType.Info, LogName, $"Rejected connection from {connection}: {reasonString}.");
        }
        ...
        protected override void Handle(Message message, MessageHeader header, Connection connection) {
            switch (header) {
                ...
                default:
                    NovaCoreLogger.Log(LogType.Warning, LogName, $"Unexpected message header '{header}'! Discarding {message.WrittenLength} bytes received from {connection}.");
                    break;
            }
        ...
        public void DisconnectClient(ushort id, Message message = null) {
            if (clients.TryGetValue(id, out Connection client)) {
                ...
                NovaCoreLogger.Log(LogType.Warning, LogName, $"Couldn't disconnect client {id} because it wasn't connected!");
        }
        ...
        public void DisconnectClient(Connection client, Message message = null) {
            if (clients.ContainsKey(client.Id)) {
                ...
                NovaCoreLogger.Log(LogType.Warning, LogName, $"Couldn't disconnect client {client.Id} because it wasn't connected!");
        }
        ...
        public void Stop() {
            ...
            StopTime();
            IsRunning = false;
            NovaCoreLogger.Log(LogType.Info, LogName, "Server stopped.");
        }
        ...
        private ushort GetAvailableClientId() {
            ...
            NovaCoreLogger.Log(LogType.Error, LogName, "No available client IDs, assigned 0!");
            return 0;
        }
        ...
        protected virtual void OnClientConnected(Connection client) {
            NovaCoreLogger.Log(LogType.Info, LogName, $"Client {client.Id} ({client}) connected successfully!");
            SendClientConnected(client);
            ClientConnected?.Invoke(this, new ServerConnectedEventArgs(client));
        }

        /// <summary>
        /// Invokes the <see cref="MessageReceived"/> event and initiates handling of the received message.
        /// </summary>
        ...
        protected virtual void OnMessageReceived(Message message, Connection fromConnection) {
            ...
            if (RelayFilter != null && RelayFilter.ShouldRelay(messageId)) {
                // The message should be automatically relayed to clients instead of being handled
                // on the server
            ...
            if (messageHandlers.TryGetValue(messageId, out MessageHandler messageHandler))
                ...
                NovaCoreLogger.Log(LogType.Warning, LogName, $"No message handler method found for message ID {messageId}!");
        }
        ...
        protected virtual void OnClientDisconnected(Connection connection, DisconnectReason reason) {
            ...
            NovaCoreLogger.Log(LogType.Info, LogName, $"Client {connection.Id} ({connection}) disconnected: {reasonString}.");
