...
namespace NovaCoreNetworking {
    ...
    internal class PendingMessage {
        ...
        /// <summary>
        /// A pool of reusable <see cref="PendingMessage"/> instances.
        /// </summary>
        private static readonly List<PendingMessage> pool = new List<PendingMessage>();

        /// <summary>
        /// The <see cref="Connection"/> to use to send (and resend) the pending message.
        /// </summary>
        ...
        /// <summary>
        /// Retrieves a <see cref="PendingMessage"/> instance from the pool. If none is available, a
        ...
        /// <summary>
        /// Returns the <see cref="PendingMessage"/> instance to the pool so it can be reused.
        /// </summary>
        private void Release() {
            ...
            // available instance than are needed, which could occur if a large burst of messages
            // has to be sent for some reason
        }
        ...
        internal void TrySend() {
            if (sendAttempts >= MaxSendAttempts) {
                ...
                if (NovaCoreLogger.IsWarningLoggingEnabled) {
                    MessageHeader header = (MessageHeader)data[0];
                    if (header == MessageHeader.Reliable)
                        NovaCoreLogger.Log(LogType.Warning, connection.Peer.LogName, $"No ack received for {header} message (ID: {Converter.ToUShort(data, 3)}) after {sendAttempts} {Helper.CorrectForm(sendAttempts, "attempt")}, delivery may have failed!");
                    else
                        NovaCoreLogger.Log(LogType.Warning, connection.Peer.LogName, $"No ack received for internal {header} message after {sendAttempts} {Helper.CorrectForm(sendAttempts, "attempt")}, delivery may have failed!");
                }
        ...
        /// Clears the message.
        /// </summary>
        /// <param name="shouldRemoveFromDictionary">
        /// Whether or not to remove the message from <see cref="Connection.PendingMessages"/>.
        /// </param>
