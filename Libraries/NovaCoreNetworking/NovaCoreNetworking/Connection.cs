...
namespace NovaCoreNetworking {
    ...
    public abstract class Connection {
        ...
        /// <summary>
        /// Whether or not the connection is currently <i>not</i> connected nor trying to connect.
        /// </summary>
        ...
        /// <summary>
        /// Whether or not the connection is currently pending (will only be <see langword="true"/>
        /// when a server doesn't immediately accept the connection request).
        /// </summary>
        ...
        /// <summary>
        /// Whether or not the connection attempt has timed out. Uses a multiple of <see
        /// cref="Peer.TimeoutTime"/> and ignores the value of <see cref="CanTimeout"/>.
        /// </summary>
        ...
        /// <summary>
        /// Messages that we have received whose sequence IDs no longer fall into <see
        /// cref="AcksBitfield"/>'s range. Used to improve duplicate message filtering capabilities.
        /// </summary>
        ...
        internal bool ReliableHandle(ushort sequenceId) {
            ...
            if (sequenceGap > 0) {
                ...
                if (sequenceGap > 64)
                    NovaCoreLogger.Log(LogType.Warning, Peer.LogName, $"The gap between received sequence IDs was very large ({sequenceGap})! If the connection's packet loss, latency, or your send rate of reliable messages increases much further, sequence IDs may begin falling outside the bounds of the duplicate filter.");
        ...
        internal void UpdateReceivedAcks(ushort remoteLastReceivedSeqId, ushort remoteAcksBitField) {
            ...
            if (sequenceGap > 0) {
                ...
                // TODO: remove? I don't think this case ever executes The latest sequence ID that
                // the other end has received is older than the previous one (out of order ack)
                ...
                // The latest sequence ID that the other end has received is the same as the
                // previous one (duplicate ack)
        ...
        internal void HandleWelcomeResponse(Message message) {
            ...
            if (Id != id)
                NovaCoreLogger.Log(LogType.Error, Peer.LogName, $"Client has assumed ID {id} instead of {Id}!");
