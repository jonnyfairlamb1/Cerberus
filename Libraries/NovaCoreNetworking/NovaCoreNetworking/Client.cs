...
namespace NovaCoreNetworking {
    ...
    /// <summary>
    /// A client that can connect to a <see cref="Server"/>.
    /// </summary>
    public class Client : Peer {
        ...
        /// cref="RTT"/>, but it is less susceptible to changing drastically due to significant—but
        /// temporary—jumps in latency.
        /// </remarks>
        public short SmoothRTT => connection.SmoothRTT;

        /// <summary>
        /// Whether or not the client is currently <i>not</i> connected nor trying to connect.
        /// </summary>
        ...
        /// <summary>
        /// Whether or not the client's connection is currently pending (will only be <see
        /// langword="true"/> when a server doesn't immediately accept the connection request).
        /// </summary>
        ...
        /// </summary>
        // Not an auto property because properties can't be passed as ref/out parameters. Could use
        // a local variable in the Connect method, but that's arguably not any cleaner. This
        ...
        /// with the server, <see cref="Connect(string, int, byte, Message)"/> must be called again.
        /// </remarks>
        ...
        /// </param>
        /// <remarks>
        /// NovaCoreNetworking's default transport expects the host address to consist of an IP and
        /// port, separated by a colon. For example: <c>127.0.0.1:7777</c>. If you are using a
        /// different transport, check the relevant documentation for what information it requires
        /// in the host address.
        /// </remarks>
        /// <returns>
        ...
        public bool Connect(string hostAddress, int maxConnectionAttempts = 5, byte messageHandlerGroupId = 0, Message message = null) {
            ...
            if (!transport.Connect(hostAddress, out connection, out string connectError)) {
                NovaCoreLogger.Log(LogType.Error, LogName, connectError);
            ...
            Heartbeat();
            NovaCoreLogger.Log(LogType.Info, LogName, $"Connecting to {connection}...");
            return true;
        }
        ...
        protected override void CreateMessageHandlersDictionary(byte messageHandlerGroupId) {
            ...
            foreach (MethodInfo method in methods) {
                ...
                if (clientMessageHandler != null) {
                    ...
                    // It's not a message handler for Client instances, but it might be one for
                    // Server instances
        ...
        protected override void Handle(Message message, MessageHeader header, Connection connection) {
            switch (header) {
                ...
                default:
                    NovaCoreLogger.Log(LogType.Warning, LogName, $"Unexpected message header '{header}'! Discarding {message.WrittenLength} bytes.");
                    break;
            }
        ...
        /// cref="Message.Release"/> can be used to manually return the message to the pool at a
        /// later time.
        /// </remarks>
        ...
        /// TData that should be sent to the client being disconnected. Use <see
        /// cref="Message.Create()"/> to get an empty message instance. Unused if the connection
        /// wasn't rejected.
        /// </param>
        ...
        protected virtual void OnConnected() {
            NovaCoreLogger.Log(LogType.Info, LogName, "Connected successfully!");
            Connected?.Invoke(this, EventArgs.Empty);
        }

        /// <summary>
        /// Invokes the <see cref="ConnectionFailed"/> event.
        /// </summary>
        ...
        protected virtual void OnConnectionFailed(RejectReason reason, Message message = null) {
            ...
            NovaCoreLogger.Log(LogType.Info, LogName, $"Connection to server failed: {reasonString}.");
        ...
        protected virtual void OnDisconnected(DisconnectReason reason, Message message) {
            ...
            NovaCoreLogger.Log(LogType.Info, LogName, $"Disconnected from server: {reasonString}.");
        ...
        protected virtual void OnClientConnected(ushort clientId) {
            NovaCoreLogger.Log(LogType.Info, LogName, $"Client {clientId} connected.");
        ...
        protected virtual void OnClientDisconnected(ushort clientId) {
            NovaCoreLogger.Log(LogType.Info, LogName, $"Client {clientId} disconnected.");
