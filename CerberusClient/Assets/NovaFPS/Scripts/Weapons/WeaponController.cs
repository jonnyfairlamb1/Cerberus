...
public class WeaponController : MonoBehaviour {
    ...
    //References
    [Tooltip("Attach your weapons in order depending of their ID")] public GameObject[] weapons;
    ...
    private void HandleAimingMotion() {
        ...
        if (!isAiming || weapon == null) mainCamera.fieldOfView = Mathf.Lerp(mainCamera.fieldOfView, GetComponent<PlayerMovement>().normalFOV, Time.deltaTime * aimingSpeed);
    }
        ...
        switch (style) {
            case 0: //hitscan
                ...
                while (i < bulletsPerFire) {
                    ...
                    StartCoroutine(NovaFPSUtilities.PlayAnim("shooting", inventory[currentWeapon].GetComponentInChildren<Animator>()));
                    if (weapon.timeBetweenShots != 0) SoundManager.Instance.PlaySound(weapon.audioSFX.firing, 0, weapon.pitchVariationFiringSFX, 0);
    ...
    private void HitscanShot() {
        ...
        Vector3 dir = NovaFPSUtilities.GetSpreadDirection(weapon.spreadAmount, mainCamera);
    ...
    private void ProjectileShot() {
        ...
        Vector3 destination = (Physics.Raycast(ray, out hit) && hit.transform.tag != "Player") ? destination = hit.point + NovaFPSUtilities.GetSpreadDirection(weapon.spreadAmount, mainCamera) : destination = ray.GetPoint(50f) + NovaFPSUtilities.GetSpreadDirection(weapon.spreadAmount, mainCamera);
    ...
    /// <summary>
    /// Moreover, NovaFPS´ FPS ENGINE also supports melee attacking
    ...
    public void MeleeAttack(float attackRange, float damage) {
        ...
        foreach (var c in col) {
            if (c.transform.GetComponent<IDamageable>() != null) {
                if (c.tag == "Critical") c.transform.parent.GetComponent<IDamageable>().Damage(dmg);
    ...
    public void SecondaryMeleeAttack() {
        CanMelee = false;
        StartCoroutine(NovaFPSUtilities.PlayAnim("hit", holsterMotionObject.GetComponent<Animator>()));
    ...
    public void FinishMelee() {
        StartCoroutine(NovaFPSUtilities.PlayAnim("finished", holsterMotionObject.GetComponent<Animator>()));
    ...
    private IEnumerator Reload() {
        ...
        StartCoroutine(NovaFPSUtilities.PlayAnim("reloading", inventory[currentWeapon].GetComponentInChildren<Animator>()));
    ...
    public void UnHolster(GameObject weaponObj) {
        ...
        StartCoroutine(NovaFPSUtilities.PlayAnim("unholster", inventory[currentWeapon].GetComponentInChildren<Animator>()));
    ...
    private void HandleUI() {
        ...
        RaycastHit hit_;
        if (Physics.Raycast(mainCamera.transform.position, mainCamera.transform.forward, out hit_, weapon.bulletRange) && hit_.transform.tag == "Enemy" || Physics.Raycast(mainCamera.transform.position, mainCamera.transform.forward, out hit_, weapon.bulletRange) && hit_.transform.tag == "Critical")
    ...
    public void SelectWeapon() {
        ...
        foreach (GameObject weapon_ in inventory) {
            ...
            if (weapon_ == inventory[currentWeapon] && weapon_ != null) {
                ...
                NovaFPSUtilities.ApplyPreset(crosshair.GetComponent<CrosshairShape>().currentPreset, crosshair.GetComponent<CrosshairShape>());
#endif
            }
        }
    ...
    private void GetInitialWeapons() {
        ...
        while (i < initialWeapons.Length) {
            var weaponPicked = Instantiate(weapons[initialWeapons[i].weaponID], weaponHolder);
            weaponPicked.transform.localPosition = weapons[initialWeapons[i].weaponID].transform.localPosition;

            inventory[i] = weaponPicked;

            if (i == currentWeapon) {
                UnHolster(weaponPicked);
            }
