...
public class Enemy : MonoBehaviour, IDamageable {
    ...
    // Coming soon
    /*[Tooltip("Shoot at the player? ")] public bool attacksPlayer;
    bool attacking;
    [Tooltip("Shooting interval")] public float shootingFireRate;
    [Tooltip("Velocity to aim towards the player")] public float rotationSpeed;*/
    ...
    public virtual void Update() {
        ...
        if (attacksPlayer)
        {
            if (!attacking) { StartCoroutine(Shoot(shootingFireRate)); }
            Vector3 dir = new Vector3(player.position.x - transform.position.x,transform.position.y, player.position.z - transform.position.z).normalized;
            Quaternion target = transform.rotation * Quaternion.FromToRotation(transform.forward, dir);
            transform.localRotation = Quaternion.Lerp(transform.localRotation, target, Time.deltaTime * rotationSpeed);
        }
        */
    }
...
public class EnemyEditor : Editor {
    ...
    public override void OnInspectorGUI() {
        ...
        EditorGUILayout.PropertyField(serializedObject.FindProperty("attacksPlayer"));
        EditorGUILayout.PropertyField(serializedObject.FindProperty("shootingFireRate"));
