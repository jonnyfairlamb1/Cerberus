    ...
    public override void Die() {
        ...
        foreach (Collider c in cols) {
            if (c.GetComponent<Rigidbody>() != null)
                c.GetComponent<Rigidbody>().AddExplosionForce(explosionForce / Vector3.Distance(c.transform.position, transform.position), transform.position, explosionRadius, 5, ForceMode.Force);
